...
import pw.zakharov.gameapi.util.SerializeUtil;
...
public class DataFile {
    ...
    /**
     * The header in the file, always set
     */
    ...
    // Attempts to convert an object we got from the config into a HashMap
    //
    // Do not parse the path here like "Arena.Players" but the actual object
    // that is found when you call get("Arena.Players")
    private Map<String, Object> getMapFromSection(Object mapOrSection) {
        try {
    ...
    // A simple reflection helper to get the field content
    ...
    // A lazy mans helper to automatically insert path prefix
    // when calling getX methods so we dont have to call
    // Arena.Players, Arena.Classes each time but only Players and Classes
    private String formPathPrefix(String myPath) {
        String path = "";
